...
import { ColumnConfig } from "../tables/SalaryTable";
...
export const AddModalForm: React.FC<AddModalFormProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    schema.forEach(({ key, required }) => {
      if (required && !formData.get(key)) {
        hasErrors = true;
        alert(`Campo "${key}" è obbligatorio!`);
      }
    });
  ...
  const getInputType = (format?: ColumnConfig['format']): React.InputHTMLAttributes<HTMLInputElement>['type'] => {
    switch (format) {
          ...
          {schema.map(({ key, label, format, required }) => (
            <div key={key} className="w-full">
              ...
              <Input 
                ...
                className="w-full"
                placeholder={format === 'currency' ? "0.00" : label}
              />
            </div>
          ))}
        </div>
